...
const Edit = () => {
  ...
  const [filter, setFilter] = useState({ authorList: [], categoryList: [] });
  ...
  useEffect(() => {
    ...
    getPostById(id).then((data) => {
      if (data) setPost({ ...data, selectedTags: data.tags.map((tag) => tag?.name).join('\r\n') });
      else setPost(initialState);
    });
    getFilter().then((data) => {
      if (data) setFilter({ authorList: data.authorList, categoryList: data.categoryList });
        ...
        <div className="row mb-3">
          ...
          <div className="col-sm-10">
            <Form.Control
              ...
              onChange={(e) => setPost({ ...post, selectedTags: e.target.value })}
            />
            <Form.Control.Feedback type="invalid">Không được bỏ trống.</Form.Control.Feedback>
          </div>
        </div>
